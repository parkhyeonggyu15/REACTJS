...
export function Greeting({ isLogin, name }) {
  ...
  return isLogin ? <h4>{name}님 환영합니다.</h4> : <h4>게스트님 안녕하세요.</h4> ;
}
...
export function StatusBadge({ status }) {
  ...
  return (<p>주문 상태 : {(() =>{if(status === "ready") return "준비중"
...
export function UserName({ name, isAdmin }) {
  ...
  return (<p>{name || "익명 사용자"}{isAdmin && (isAdmin)}</p>);
}
...
export function TrafficLight({ color }) {
  ...
  switch(color){
    case "red" : return <p>정지</p>
    case "green" : return <p>출발</p>
    case "yellow" : return <p>주의</p>
    default : return <p>신호 없음</p>
  };
}
