...
export function DiscountLine({ name, price, rate }) {
  ...
  const 할인가 = price * (100 - rate) / 100;
  return <p>정가{price}원 -> {rate}% 할인가 {할인가}원</p>;
}
...
// ---------------------------------------------------------------------
export function StockStatus({ name, stock, status }) {
  // {TODO} if / else if / else 로 status 를 정한 뒤 <p> 로 return 하세요.
  if (stock === 0) {
    status = "품절"
  } else if (stock <= 5) {
    status = "마감임박(" + stock + "개 남음)"
  } else {
    status = "구매가능"
  };
